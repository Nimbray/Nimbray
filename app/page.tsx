...
function threadsForStorage(list: Thread[]) {
  return list.map((thread) => ({
    ...
    messages: thread.messages.map((message) => ({
      ...message,
      images: message.images?.map(({ dataUrl, ...image }) => ({ ...image, dataUrl: "" }))
    }))
  }));
}
...
function isClientSafetyOverride(text: string, messages: Message[] = []) {
  ...
  const directDanger = /\b(je veux mourir|je vais mourir|je veux me tuer|je vais me tuer|je vais me suicider|je vais me faire du mal|je veux me faire du mal|plus envie de vivre|mettre fin a mes jours|j ai envie d en finir|je vais tuer|je veux tuer|je vais faire du mal|je veux faire du mal|comment cacher un corps|comment fabriquer une bombe|poison mortel|explosif)\b/.test(q);
  if (directDanger) return true;

  const farewell = /^(adieu|dernier message|bye definitif|je pars pour toujours|c est fini|cest fini)$/.test(q);
  const riskyContext = /\b(je vais tuer|je veux tuer|faire du mal|me faire du mal|mourir|me tuer|suicide|suicider|disparaitre|adieu|arme|couteau|j en peux plus|je n en peux plus|ne reponds plus|silence|tais toi|arrete)\b/.test(recent);
  return farewell && riskyContext;
}
...
export default function HomePage() {
  ...
  async function send(customText?: string, overrideMessages?: Message[]) {
    ...
    const safetyOverride = isClientSafetyOverride(text, baseMessages);

    if (silenceMode && !isResumeRequest(text) && !safetyOverride) {
      setLoading(false);
      return;
    }

    if (silenceMode && safetyOverride) {
