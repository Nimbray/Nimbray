...
export function detectIntent(text: string): ModelIntent {
  ...
  if (/\b(strategie|business plan|plan d action|decision|compare|comparer|architecture|roadmap|lancer|startup|rentable|investissement|produit|saas|backend|frontend|agent|agents|sprint|vercel|supabase)\b/.test(q)) return "reasoning";
